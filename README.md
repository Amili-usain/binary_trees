...
| [1-binary_tree_insert_left.c](./1-binary_tree_insert_left.c) | <ul><li>A function that inserts a node as the left-child of another.</li><li>Returns a pointer to the new node, or `NULL` on failure.</li><li>If the given `parent` already contains a left node, the new node takes its place and the old left-child becomes the left-child of the new node.</li></ul> |
